...
module.exports = function(grunt) {
    ...
    PROJECT.dockerRepo = `986433031334.dkr.ecr.us-east-1.amazonaws.com/${PROJECT.unscopedName}`;
    ...
    grunt.initConfig({
        ...
        env: {
            test: {
                JWT_PUBLIC_KEY:
                    '-----BEGIN PUBLIC KEY-----\r\n' +
                    'MIIBITANBgkqhkiG9w0BAQEFAAOCAQ4AMIIBCQKCAQB6p2ervjCT2Nfm8mR2bv02\r\n' +
                    'j58SJ8uNcPidiU/ZsN3vO2YN8bV27XjP03CfRq2p9nd2T176ldG75WI/Q+EAn7W2\r\n' +
                    'ykoWhqma9yWQXDECLelvxn3lKagjtf7uI+mNLusDcBsyIayTZ5NlHb6/1PCa82oi\r\n' +
                    'T9W//M8UVNwE1h4ytKt/g+jyOK5oNpCCK6zVZEPpIUVOPgihUv7tyRAMxNPUzF5C\r\n' +
                    'OCubg63pKxi7IkoLHvi3EZ4TidyOc7qQCPmTcwIXv3PlVNjsWyeuo30T/KYe36MX\r\n' +
                    'jSgMBJowKcbBaJYlNmf+bIsGA1ujS+KeRVNhFAM5exoQgYdxtrgxxfGU9lkqhoId\r\n' +
                    'AgMBAAE=\r\n' +
                    '-----END PUBLIC KEY-----',
                NODE_ENV: 'test',
                JWT_ALGORITHM: 'RS256',
        ...
        tslint: {
            ...
            default: {
                src: [SRC.getAllFilesPattern('ts'), TEST.getAllFilesPattern('ts')],
            },
        },
        ...
        shell: {
            dockerBuild: {
                command:
                    `docker build --rm --tag ${
                        PROJECT.dockerTag
                    } ${__dirname} --build-arg APP_NAME=${camelcase(PROJECT.unscopedName)} ` +
                    ` --build-arg NPM_USER=${grunt.option(
                        'npm-user'
                    )} --build-arg NPM_TOKEN=${grunt.option('npm-token')}`,
            },
            dockerPublish: {
        ...
        typedoc: {
            default: {
                options: {
                    module: 'commonjs',
                    out: DOCS.getFilePath(`${PROJECT.appName}/${PROJECT.version}`),
        ...
        watch: {
            ...
            sourceFiles: {
                files: [SRC.getAllFilesPattern('ts'), TEST.getAllFilesPattern('ts')],
                tasks: [],
            },
        },
    ...
    grunt.registerTask('test', 'Executes tests against sources', (target) => {
        ...
        if (['unit', 'api'].indexOf(target) >= 0) {
            ...
            if (typeof testSuite === 'string' && testSuite.length > 0) {
                ...
                const path = WORKING.getChild(`test/${target}`).getFilePath(testSuite);
    ...
    grunt.registerTask(
        ...
        (target) => {
            ...
            const tasks = validTasks[target];
            const watchTask =
                ['docs', 'lint', 'build'].indexOf(target) >= 0 ? 'sourceFiles' : 'workingFiles';
